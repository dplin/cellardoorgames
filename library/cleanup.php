...
function foundationpress_start_cleanup() {
    ...
    add_filter('nav_menu_css_class', 'dl_css_attributes_filter', 100, 1);
    add_filter('nav_menu_item_id', 'dl_css_attributes_filter', 100, 1);
    add_filter('page_css_class', 'dl_css_attributes_filter', 100, 1);
}
...
function foundationpress_cleanup_head() {
	...
	// Remove Emoji script from header (v4.2+)
...
function dl_css_attributes_filter($var) {
  return is_array($var) ? array_intersect($var, array('has-dropdown')) : '';			// Only keeps class name "has-dropdown"
}
